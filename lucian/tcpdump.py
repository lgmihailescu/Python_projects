...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Capture DNS queries for specified zones')
    parser.add_argument('zone', nargs='+', help='a DNS zone to be captured')
    parser.add_argument('--output', '-o', help='write dump to file')

    args = parser.parse_args()
    print >> sys.stderr, 'Capturing DNS requests for zones %s' % args.zone
    ...
    try:
        if args.output:
            ...
            out_file = None
        sniff(iface='eth0', filter='udp src port 53', prn=scanner, store=0)
